...
var evalPrices = function() {
	...
	var skidka = 0.6; // 0 = 0% скидка, 0.1 = 10% скидка
	var rise = 0.0;
	
	// Разрешить вычисление коэффициентов "тепло, тишина, дизайн"
	var koefsEnable = true;
...
//console.log(price);	
	...
	var timer = null, intv = 25, msecs = 750, ticks = msecs / intv, totalTicks = ticks,
		currents = { d: calc.prices.delivery, m: calc.prices.installation, o: calc.prices.otdelka, p: calc.prices.total, cr: calc.prices.credit },
		deltas = { d: (delivery-currents.d)/ticks, m: (installation-currents.m)/ticks, o: (otdelka-currents.o)/ticks, p: (price-currents.p)/ticks, cr: (credit-currents.cr)/ticks};
	timer = setInterval(function() {
		if (ticks<=0) clearInterval(timer);
		tickKoef = totalTicks - ticks;
		$("#calc #calc-prices")
			.find("#delivery-price").html(formatNum(Math.round(currents.d+tickKoef*deltas.d))).end()
